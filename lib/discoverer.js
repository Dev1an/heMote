...
const broadcastMessage = new Buffer.from("00010028000D0000000000000026186A7813A9FE3A59000020111E11231F1E1913000000010000000000000000000000FFF00024002000210022002300250028004000410042004400A500A600A700A800AD00B300B700B8FFFF0FE5", "hex");
...
function messageListener(data) {
	...
	if (hexDataString.search('00a8001043414d3a') !== -1) {
		...
		const messageFields = {
			...
			'address': hexDataString.search("00200004")/2+4,
		...
		for (property in {'address':0, 'defaultGateway':0, 'primaryDNS':0, 'secondaryDNS':0, 'subnetMask':0}) {
