...
const panaPtzCommands = [
    ...
    { commandResponse: 'qSV3V', commandRegex: /(^qSV3V)(.*)/gm, commandName: 'Version information notification'},
    { commandResponse: 'lPI', commandRegex: /(^lPI)(.*)/gm, commandName: 'Lens information'},
    { commandResponse: 'lPI', commandRegex: /(^q)(\d\d)/gm, commandName: 'Preset playback'},
]
...
function parse(data) {
    ...
    // We iterate through all known commands.
    panaPtzCommands.forEach(command => {

        // We create an array with all the occurrences of this command
        const matches = [...data.matchAll(command.commandRegex)]

        // If at least on occurrence is found
        if (matches.length) {

            // Keep only the "status" message part of each command
            const rawStatus = matches.map(match => [...match].slice(2))

            // Convert the status message
            if (command.convert) {
                let result = [];
                rawStatus.forEach(element => {
                    const call = command.convert(...element)
                    result = result.concat(call)
                })
                status[command.commandName] = result
            }

            // If there is no converter, return the raw status message
...
//const exampleData = fs.readFileSync('../examples/data/camdataHE130_121.txt', 'utf8')
